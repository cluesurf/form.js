...
async function makeBackTest(base) {
    const list = [];
    list.push(`import { z } from 'zod'`);
    list.push(`import { Back } from './form.js'`);
    for (const name in base) {
        list.push(`export const ${pascal(name)}Test: z.ZodType<Back.Form.${pascal(name)}> = z.object({`);
        const form = base[name];
        if (!form) {
            continue;
        }
...
function makeZodFoot(base, form) {
    const list = [];
    ...
    list.push(`}`);
    list.push(`export function need<Name extends ${form}.Name>(bond: unknown, form: Name): asserts bond is ${form}.Base[Name] {`);
    list.push(`const test = Test[form]`);
    list.push(`test.parse(bond)`);
    list.push(`}`);
    list.push(`export function test<Name extends ${form}.Name>(bond: unknown, form: Name): bond is ${form}.Base[Name] {`);
    list.push(`const test = Test[form]`);
    list.push(`return test.safeParse(bond).success`);
    list.push(`}`);
    list.push(`export function take<Name extends ${form}.Name>(bond: unknown, form: Name): ${form}.Base[Name] {`);
    list.push(`const test = Test[form] as z.ZodType<${form}.Base[Name]>`);
    list.push(`return test.parse(bond)`);
    list.push(`}`);
    return list;
}
async function makeFaceForm(base) {
    const list = [];
...
async function makeBackForm(base) {
    const list = [];
