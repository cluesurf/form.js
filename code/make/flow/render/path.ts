...
import type { BaseCtx } from './registry'
...
export function evaluatePath(
  ...
  for (let i = 0; i < node.path.length; i += 1) {
    ...
    if (i === 0) {
      ...
      current = ctx.scope.get(seg.name)
      if (seg.safe && current == null) return null
      continue
    }
    if (current == null) return null
    current = evaluateSeg(seg, current, ctx, evaluateNode)
    ...
    case 'index': {
      const i =
        typeof seg.value === 'number'
          ? seg.value
          : Number(evaluateNode(seg.value, ctx))
      ...
      const rise =
        seg.rise == null
          ...
          : typeof seg.rise === 'number'
            ? seg.rise
            : Number(evaluateNode(seg.rise, ctx))
      const fall =
        seg.fall == null
          ? current.length
          : typeof seg.fall === 'number'
            ? seg.fall
            : Number(evaluateNode(seg.fall, ctx))
